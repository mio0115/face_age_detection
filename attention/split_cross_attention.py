...
class CrossAttention(tf.keras.layers.Layer):
    ...
    def _split_heads(self, x, seq_len: int):
        ...
        x = tf.reshape(x, shape=(batch_size, seq_len, self.heads_num, self.per_head_dim))
    ...
    def call(self, obj_queries, query_obj_pos, encoder_output, key_encoder_pos):
        ...
        obj_queries = tf.ensure_shape(obj_queries, shape=(None, )+self.input_shape, name="check_sc_attn_input_shape")

        # query
        obj_queries = self._split_heads(obj_queries, 5)
        query = tf.concat([self._proj_to_query(obj_queries), self._split_heads(query_obj_pos, 5)], axis=-1) # to 512 (8, 32) -> (8, 64)

        # key
        encoder_output = self._split_heads(encoder_output, 49)
        key = tf.concat([self._proj_to_key(encoder_output), self._split_heads(key_encoder_pos, 49)], axis=-1) # ()
        
        # value
        value = self._proj_to_value(encoder_output)
        
        a_cross = tf.nn.softmax(tf.matmul(query, tf.transpose(key, perm=[0, 1, 3, 2])) / tf.sqrt(tf.cast(self.embedding_dim, dtype=tf.float32)), axis=-1)
        o = tf.matmul(a_cross, value) # (8, 512)
        o = tf.transpose(o, perm=[0, 2, 1, 3])
        o = tf.cast(o, dtype=tf.float32)
        o = tf.reshape(o, shape=(batch_size, self.sequence_length, self.heads_num * self._d_v)) # 8 * 512 = 4096
