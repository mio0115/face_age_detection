...
def load_data(cluster_size: int = 5000, num_cls: int = 8, clusters: int = 6):
    ...
    for idx in range(clusters):
        logits = np.load(
            os.path.join("/", "workspace", "data", "numpy_array", f"imdb_1_{idx}.npy")
        )
        ...
        corr_labels = _padding(
            labels.loc[st:end, label_col + oh_label_col + coord_col], padding_size
        )

        data.append(logits)
        targets.append(tf.convert_to_tensor(corr_labels, dtype=tf.float32))

    targets = tf.reshape(
        tf.stack(targets, axis=0),
        shape=(-1, len(label_col) + len(oh_label_col) + len(coord_col)),
    )
...
if __name__ == "__main__":
    ...
    for batch in dataset:
        ...
        print(
            tf.reshape(
                tf.cast(tf.io.decode_raw(img, tf.uint8), tf.float32), (-1, 224, 224, 3)
            )
        )
        idx += 1
        if idx == 2:
