...
def _cover_zeros(cost_matrix: tf.Tensor) -> list:
    ...
    while np.count_nonzero(matrix) < n ** 2:
        nonzero_cnt = np.concatenate((np.count_nonzero(matrix, axis=1), np.count_nonzero(matrix, axis=0)))
...
def tf_linear_sum_assignment(cost_matrix):
    return tf.numpy_function(func=linear_sum_assignment, inp=[cost_matrix], Tout=[tf.int64, t])


class HungarianMatcher(tf.keras.layers.Layer):
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, num_cls: int = 8):
    ...
    def call(self, outputs, targets):
        ...
        out_prob = tf.nn.sigmoid(tf.reshape(outputs['pred_obj_cls'], shape=[batch_size * num_objects, self._num_cls]), axis=-1)
        out_bbox = tf.reshape(outputs['pred_boxes'], shape=[batch_size*num_objects, 4])
        ...
        neg_cost_class = (1 - param['neg_coeff']) * (out_prob ** param['power']) * tf.nn.log(-(1 - out_prob + 1e-8))
        pos_cost_class = param['neg_coeff'] * ((1 - out_prob) ** param['power']) * tf.nn.log(-(out_prob + 1e-8))
...
class SingleTargetMatcher(tf.keras.layers.Layer):
    def __init__(self, cost_class: float = 1.0, cost_bbox: float = 1.0, cost_iou: float = 1.0, num_class: int = 8):
    ...
    def call(self, outputs, tgt_labels, tgt_bbox, is_xyxy=tf.constant(False, dtype=tf.bool)):
        batch_size, num_objects = tf.shape(outputs['pred_obj_cls'])[0], tf.shape(outputs['pred_obj_cls'])[1]

        out_prob = tf.sigmoid(tf.reshape(outputs['pred_obj_cls'], shape=(batch_size*num_objects, self._num_cls)))
        out_bbox = tf.reshape(outputs['pred_boxes'], shape=(batch_size*num_objects, 4))

        alpha, gamma = 0.25, 2.0
        neg_cost_cls = (1 - alpha) * (out_prob ** gamma) * (-tf.math.log(1 - out_prob + 1e-8))
        pos_cost_cls = alpha * ((1 - out_prob) ** gamma) * (-tf.math.log(out_prob + 1e-8))
        
        cost_cls = tf.gather(pos_cost_cls, tgt_labels, axis=1) - tf.gather(neg_cost_cls, tgt_labels, axis=1)
        cost_cls = tf.squeeze(cost_cls, axis=-1)

        # check the form of target_bbox coordinate
        out_bbox = tf.cond(is_xyxy, true_fn=lambda: out_bbox, false_fn=lambda: from_cxcyhw_to_xyxy(out_bbox))
        l1_bbox_cost = smooth_l1_dist(
            bbox=out_bbox,
            tgt_bbox=tgt_bbox
        )
        # Note that iou is not "cost"
        # Higher IoU represents more fit to target.
        iou_bbox_cost = -1 * get_iou(
            bbox=out_bbox,
            tgt_bbox=tgt_bbox
        )
        ...
        cost = self._cost_bbox * l1_bbox_cost + self._cost_cls * cost_cls + self._cost_iou * iou_bbox_cost

        cost = tf.reshape(cost, shape=(batch_size, num_objects, -1))

        tgt_sizes = [1] * 8
        
        return [tf.argmin(cost[batch_idx], axis=0, output_type=tf.int32)
                for batch_idx, cost in enumerate(
                    tf.split(cost, 
                             num_or_size_splits=tgt_sizes, 
                             axis=-1))
                ]
