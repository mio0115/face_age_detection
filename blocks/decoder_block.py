...
class DecoderBlock(tf.keras.layers.Layer):
    def __init__(self, object_queries_shape, lambda_: float = 0.5, hidden_dim: int = 512, **kwargs):
        ...
        self._cls_branch = ClsRegBranch(object_queries_shape=(object_queries_shape[0], hidden_dim//2))
        self._reg_branch = ClsRegBranch(object_queries_shape=(object_queries_shape[0], hidden_dim//2))
    ...
    def call(self, object_queries: tf.Tensor, encoder_output: tf.Tensor, obj_centers: tf.Tensor, obj_pos_encoding: tf.Tensor):
        batch_size = tf.shape(object_queries)[0]
        object_queries = tf.ensure_shape(object_queries, shape=(None,)+self.object_queries_shape, name='check_decoder_input')

        obj_pos_encoding = gen_sineembed_for_position(pos_tensor=obj_centers, d_model=512)
        ...
        query_obj_pos = tf.ensure_shape(query_obj_pos, shape=(None, self.object_queries_seq_len, 256))
        ...
        o = self._lambda * self.layer_norm1(object_queries + o1) + \
            (1 - self._lambda) * self.layer_norm2(object_queries + o2)
        o = tf.ensure_shape(o, shape=(None, self.object_queries_seq_len, self.object_queries_embedding_dim))
        ...
        o = tf.ensure_shape(
            ...
            shape=(None, self.object_queries_seq_len, self.object_queries_embedding_dim)
        )
