...
class MiniDetector(tf.keras.layers.Layer):
    def __init__(self, input_shape, cls_num: int, top_k: int, hidden_dim: int = 256, **kwargs):
        ...
        self._cls_conv = Sequential([
            Conv2D(filters=hidden_dim, kernel_size=(3, 3), strides=1, padding="same")
            for _ in range(4)
        ...
        self._reg_head = Sequential([
            ...
            Dense(units=4, activation='sigmoid') # use sigmoid instead of relu to restrict the coord between 0 and 1.
    ...
    def call(self, inputs):
        ...
        cls_x = tf.reshape(cls_x, shape=(batch_size, self.sequence_length, self.embedding_dim))
        cls_features = cls_x
        # shape of x after cls_head is (batch_size, sequence_length, cls_num)
        cls_scores = self._cls_head(cls_x)

        pos_query = gen_sineembed_for_position(tf.reshape(inputs, shape=(batch_size, 49, self.input_shape[-1])), d_model=self.input_shape[-1])
        pos_query = tf.reshape(pos_query, shape=(batch_size, self.input_height, self.input_width, self.input_shape[-1]))
        pos_features = self._pos_conv(pos_query)
        pos_features = tf.reshape(pos_features, shape=(batch_size, self.sequence_length, self.embedding_dim))
        
        pos_center_offset = self._pos_head(pos_features)
        pos_center_offset = tf.concat(
            [
                pos_center_offset, 
                tf.zeros(shape=(batch_size, self.sequence_length, 2))
            ], 
            axis=-1
        )
        ...
        reg_x = tf.reshape(reg_x, shape=(batch_size, self.sequence_length, self.embedding_dim))
        reg_features = reg_x
        ...
        top_k_pos = gen_sineembed_for_position(pos_tensor=top_k_centers, d_model=2*self._hidden_dim)
