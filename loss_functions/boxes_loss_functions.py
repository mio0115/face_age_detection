...
def _penalty_of_weight_height(bbox, limit=1.):
    ...
    penalty_height = tf.where(tf.less(center_y - height/2, 0.), tf.abs(center_y - height/2), 0.) + \
        tf.where(tf.greater(center_y + height/2, limit), tf.abs(center_y + height/2), 0.)
    
    penalty_width = tf.where(tf.less(center_x - width/2, 0.), tf.abs(center_x - width/2), 0.) + \
        tf.where(tf.greater(center_x + width/2, limit), tf.abs(center_x + width/2), 0.)
