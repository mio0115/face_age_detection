...
class PairSelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, content_embedding, obj_pos_encoding, top_k_centers):
        ...
        a2 = tf.keras.activations.softmax(
            (a2_l + a2_r)
            / tf.sqrt(tf.cast(2 * self.input_embedding_dim, dtype=tf.float16))
        )
...
def _get_pairs(top_k_coord: tf.Tensor):
    ...
    bbox_iou = inter_area / bbox_union_area - tf.eye(
        num_objects, batch_shape=[batch_size], dtype=tf.float16
    )
    ...
    pair_idx = tf.stack(
        [
            tf.broadcast_to(
                tf.range(start=0, limit=num_objects)[tf.newaxis, ...],
    ...
    bbox_l1 = (top_k_coord[..., 2] - top_k_coord[..., 0]) + (
        top_k_coord[..., 3] - top_k_coord[..., 1]
    )
    ...
    bbox_l1_pair = tf.stack(
        [bbox_l1, tf.gather(bbox_l1, pair_idx[..., 1], batch_dims=1)], axis=-1
    )
    ...
    correct_order = tf.where(
        (bbox_l1_pair[..., 0] >= bbox_l1_pair[..., 1])[..., tf.newaxis],
        pair_idx,
        tf.reverse(pair_idx, axis=[-1]),
    )
