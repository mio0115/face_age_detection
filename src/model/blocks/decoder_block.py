...
from ..attention.pair_self_attention import PairSelfAttention, PairSelfAttentionV2
...
class DecoderBlock(tf.keras.layers.Layer):
    ...
    def call(
        ...
        obj_pos_encoding = gen_sineembed_for_position(
            pos_tensor=obj_centers, d_model=self._channels
        )
        ...
        o2 = self._pair_attn_layer(query=q, key=k, value=v, top_k_centers=obj_centers)
        ...
        cls_output = self._cls_branch(inputs=o_cls, query=q_cls, key=k, value=v)
        reg_output = self._reg_branch(inputs=o_reg, query=q_reg, key=k, value=v)
        o = tf.ensure_shape(
...
class ClsRegBranchV2(tf.keras.layers.Layer):
    def __init__(self, attn_input_shape, attn_output_shape, hidden_dim: int = 256):
        ...
        self.self_attn_layer = SelfAttentionV2(
            heads_num=8, input_shape=attn_input_shape, output_shape=attn_output_shape
        )
