...
class ObjDetSplitTransformer(tf.keras.layers.Layer):
    def __init__(self, input_shape, num_cls: int, hidden_dim: int = 512, num_encoder_blocks: int = 1, num_decoder_blocks: int = 6, top_k: int = 5):
        super(ObjDetSplitTransformer, self).__init__()

        self._resnet_backbone = tf.keras.applications.ResNet50(weights='imagenet',
                                                              include_top=False,
        ...
        self._mini_detector = MiniDetector(input_shape=(7, 7, hidden_dim), cls_num=num_cls, top_k=top_k, name="mini-detector")
        ...
        self._reg_ffn = tf.keras.layers.Dense(units=4, activation="sigmoid") # use sigmoid instead of relu to restrict the output between 0 and 1.
    ...
    def call(self, inputs):
        ...
        top_k_proposals, top_k_centers, top_k_pos, all_proposals = self._mini_detector(x)
        ...
        encoder_output = tf.reshape(self._reduce_dim_2(tf.reshape(encoder_output, shape=(-1, 7, 7, 512))), shape=(-1, 49, 256)) # embedding dimension from 512 to 256
        ...
        idx = tf.argmax(tf.reduce_max(cls_output, axis=-1), axis=-1, output_type=tf.int32)
...
def train_one_step(model, optimizer, images, targets, num_cls: int = 8):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tgt_boxes = tf.gather(targets, [9, 11, 10, 12], axis=-1) / 224. # to min_x, min_y, max_x, max_y
    ...
    gradients_mini_det = [grad if grad is not None else tf.zeros_like(var) for grad, var in zip(gradients_mini_det, model.trainable_variables)]
    clipped_gradients = [tf.clip_by_value(grad, -1.0, 1.0) for grad in gradients_mini_det]
    optimizer.apply_gradients(zip(clipped_gradients, model.trainable_variables))
...
@tf.function
def train_one_stepV2(model, optimizer, images, targets: tf.RaggedTensor, num_cls: int = 9, padding_to_k: int = 5, batch_size: int = 8):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        tgt_labels = tf.gather(targets, [0], axis=-1).to_tensor(default_value=num_cls-1, shape=(batch_size, padding_to_k, 1))
        tgt_oh_labels = tf.gather(targets, [1, 2, 3, 4, 5, 6, 7, 8], axis=-1).to_tensor(default_value=0., shape=(batch_size, padding_to_k, num_cls-1)) # a class is for empty object
        tgt_oh_labels = padding_oh_labels(tgt_oh_labels)
        tgt_boxes = tf.gather(targets, [9, 11, 10, 12], axis=-1).to_tensor(default_value=0.0, shape=(batch_size, padding_to_k, 4)) / 224. # to min_x, min_y, max_x, max_y
